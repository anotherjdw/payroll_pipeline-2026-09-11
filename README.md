...
the `AWS_ROLE_ARN_DEV` / `AWS_ROLE_ARN_PROD` secrets, and the `AWS_REGION` variable that
`deploy-dev.yml` / `deploy-prod.yml` need. Each environment has one role, used for both
the pull-request plan and the gated apply. It does not set branch protection on `main` —
that is the one step it deliberately leaves for you to configure by hand.
<!-- AUTO:END deploying -->
